//! Command-line front end for the DNP3 DPI decoder.
//!
//! Reads hex-encoded link frames (one per line, whitespace tolerated, `#`
//! comments ignored) from a file or standard input and writes one normalized
//! JSON event per decoded application message to standard output.
...
fn usage() {
    eprintln!("dnp3-dpi — decode DNP3 link frames into normalized JSON events");
    eprintln!();
    eprintln!("USAGE: dnp3-dpi [FILE]");
    eprintln!();
