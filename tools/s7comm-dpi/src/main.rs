//! Command-line front end for the S7comm DPI decoder.
//!
//! Reads hex-encoded TPKT frames (one per line, whitespace tolerated, `#`
//! comments ignored) from a file or standard input and writes one normalized
//! JSON event per decoded S7 job or response to standard output.
...
fn usage() {
    eprintln!("s7comm-dpi — decode S7comm frames into normalized JSON events");
    eprintln!();
    eprintln!("USAGE: s7comm-dpi [FILE]");
    eprintln!();
    eprintln!("Reads hex-encoded frames from FILE, or standard input when omitted.");
}
