...
fn example_events() -> Vec<opcua_dpi::Event> {
    ...
    for line in text.lines() {
        ...
        for frame in parse_stream(&bytes) {
            events.push(frame.expect("example frame parses").event());
        }
    }
    events
}
...
fn example_frames_cover_the_detection_rules() {
    let services: Vec<Option<String>> = example_events()
        .into_iter()
        .map(|event| event.opcua_service)
        .collect();
    // WriteRequest, CallRequest and BrowseRequest: one frame per rule.
    for expected in ["WriteRequest", "CallRequest", "BrowseRequest"] {
        assert!(
            services
                .iter()
                .any(|service| service.as_deref() == Some(expected)),
            "examples/frames.hex no longer decodes {expected}: {services:?}"
        );
    }
}
